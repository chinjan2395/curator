<template>
  <header class="flex items-center gap-3 px-5 py-3 bg-one-surface border-b border-one-divider shrink-0">
    <!-- Logo -->
    <div class="flex items-center gap-2 mr-3">
      <img src="/icons.svg" alt="Curator" class="w-7 h-7 rounded-xs-card" />
    ...
    <nav class="flex items-center gap-1 flex-1">
      ...
      <div class="relative" ref="workspacesDropdownRef">
        ...
        >
          ...
          Workspaces
          <svg class="w-3 h-3 ml-0.5 text-one-muted" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd"/></svg>
        </button>
        <div
          v-if="showWorkspacesDropdown"
          class="absolute top-full left-0 mt-2 w-52 rounded-sm-card bg-one-surface shadow-panel py-1.5 z-20"
        >
          <router-link
            to="/workspaces"
            class="dropdown-item"
          ...
          <div v-if="workspaces.list.length" class="border-t border-one-divider mt-1 pt-1">
            <router-link
              v-for="w in workspaces.list"
              :key="w.id"
              :to="`/workspaces/${w.id}/feeds`"
              class="dropdown-item truncate"
              :class="{ 'dropdown-item-active': Number($route.params.workspaceId) === w.id }"
              @click="showWorkspacesDropdown = false"
            >
              <svg class="w-3.5 h-3.5 shrink-0 text-one-muted" viewBox="0 0 20 20" fill="currentColor">
          ...
          <div v-else class="px-3 py-2 text-sm-pro text-one-muted">No workspaces</div>
        </div>
      </div>
    ...
    <div class="flex items-center gap-2 ml-auto">
      <button
        type="button"
        class="p-1.5 rounded-xs-card text-one-muted hover:text-one-sub hover:bg-one-bg transition-colors"
      ...
      <div class="relative" ref="profileDropdownRef">
        <button
          type="button"
          class="flex items-center gap-2 rounded-full bg-one-bg px-3 py-1.5 hover:bg-[#E8E8EA] transition-colors"
          @click="showProfileDropdown = !showProfileDropdown"
        >
          <div class="h-6 w-6 rounded-full bg-one-primary text-white text-xs font-bold flex items-center justify-center">
            {{ userInitials }}
          </div>
          <span class="text-sm-pro font-semibold text-one-text max-w-[120px] truncate">{{ auth.user?.name || 'User' }}</span>
          <svg class="w-3.5 h-3.5 shrink-0 text-one-muted" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="m19 9-7 7-7-7" /></svg>
        </button>
        <div
          v-if="showProfileDropdown"
          class="absolute right-0 top-full mt-2 w-48 rounded-sm-card bg-one-surface shadow-panel py-1.5 z-20"
        >
...
.topnav-item {
  @apply relative flex items-center gap-1.5 px-3 py-1.5 rounded-xs-card text-sm-pro text-one-sub font-medium hover:bg-one-bg hover:text-one-text transition-colors;
}
.topnav-item-active {
  @apply bg-one-primary text-white;
}
.topnav-item-active .nav-icon {
  @apply text-white;
}
.dropdown-item {
  @apply flex items-center gap-2 px-3 py-2 text-sm-pro text-one-text hover:bg-one-bg transition-colors;
}
.dropdown-item-active {
  @apply text-one-primary font-semibold;
}
</style>
