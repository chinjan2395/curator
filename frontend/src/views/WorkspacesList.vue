<template>
  <div class="space-y-4">
    ...
    <div v-if="workspaces.loading" class="surface-card flex items-center gap-2 text-sm-pro text-one-sub px-5 py-4">
      <span class="inline-block w-4 h-4 border-2 border-one-divider border-t-one-primary rounded-full animate-spin" />
      Loading…
    </div>
    <div v-else-if="workspaces.error" class="text-sm-pro text-rose-600">{{ workspaces.error }}</div>

    <!-- Onboarding empty state -->
    <div v-else-if="!workspaces.list.length" class="surface-card p-8 max-w-xl mx-auto text-center space-y-5">
      <div class="text-4xl">🚀</div>
      <div>
        <h2 class="text-lg-pro text-one-text mb-1">Welcome to Curator</h2>
        <p class="text-sm-pro text-one-sub">Get your first feed live in 4 steps.</p>
      </div>
      <ol class="text-left space-y-3 text-sm-pro text-one-text">
        <li class="flex items-start gap-3">
          <span class="onboard-step">1</span>
          <span><strong>Create a workspace</strong> — a container for one site or project.</span>
        </li>
        <li class="flex items-start gap-3">
          <span class="onboard-step">2</span>
          <span><strong>Add feeds</strong> — connect YouTube, Instagram, TikTok, RSS, and more.</span>
        </li>
        <li class="flex items-start gap-3">
          <span class="onboard-step">3</span>
          <span><strong>Curate</strong> — approve the posts you want shown publicly.</span>
        </li>
        <li class="flex items-start gap-3">
          <span class="onboard-step">4</span>
          <span><strong>Publish</strong> — copy the embed snippet and paste it into any website.</span>
        </li>
      </ol>
      ...
      <table class="w-full text-left">
        ...
        <tbody class="divide-y divide-one-divider">
          <tr v-for="w in workspaces.list" :key="w.id" class="table-tr">
            <td class="table-td font-semibold text-one-text">{{ w.name }}</td>
            <td class="table-td">
              <div class="flex items-center gap-2">
                ...
                <button type="button" class="action-link action-link--destructive" @click="confirmDelete(w)">Delete</button>
              </div>
            </td>
          </tr>
        </tbody>
